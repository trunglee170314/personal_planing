  ...
  if to_regclass('public.push_subscriptions') is not null then
    if exists (
      ...
      where table_schema='public' and table_name='push_subscriptions'
        and column_name='auth_secret'
    ) and not exists (
      select 1 from information_schema.columns
      where table_schema='public' and table_name='push_subscriptions'
        and column_name='auth'
    ) then
      alter table public.push_subscriptions rename column auth_secret to auth;
    end if;
  end if;
end;
$$;
...
begin
  if exists (
    ...
    where disabled_at is null and status in ('disabled','invalid');
  end if;
end;
$$;
...
drop policy if exists push_delivery_log_own_rows on public.push_delivery_log;
